...
// Let us create a logical and physical structure very different from the cartesian plane one
...
// The widgets used here are a button, a textfield, two panels, a split pane, and a frame - top level window.
// If we want a widget to be displayed in another widget, then we must make it a child of the latter.
// Or conversely, a child of a widget is displayed within the latter.
// Every displayed widget must have a frame ancestor, otherwise it is not displayed.
...
public class WindowTreeCreator {
	...
	// When the program stops, verify the containment relationship on the screen among widgets and their children.
	// Now verify the parent child relationship itself. Start with the splitPane. Each of these
	// widgets has an instance variable called component, which can be expanded to see the children. 
